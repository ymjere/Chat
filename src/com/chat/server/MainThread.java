...
public class MainThread implements Runnable {
	  ...
	  private MainServ _mainlaServ; // pour utilisation des méthodes de la classe principale
	  ...
	  MainThread(Socket s, MainServ mainServ) // le param s est donnée dans BlablaServ par ss.accept()
	  {
	    _mainlaServ=mainServ; // passage de local en global (pour gestion dans les autres méthodes)
	        ...
	        else if(!message.equalsIgnoreCase("")) // juste une vérification de principe
	        {
	          if(charCur[0]=='\u0000') // le dernier caractère était '\u0000' (char de terminaison nulle)
	          {
	        	// on envoi le message en disant qu'il faudra concaténer '\u0000' lors de l'envoi au client
	        	  ...
	        	  _mainlaServ.sendAll(message,""); // sinon on envoi le message à tous
	          } 
